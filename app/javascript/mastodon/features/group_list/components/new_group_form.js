...
class NewGroupForm extends React.PureComponent {
  ...
  handleClick = () => {
    ...
    this.setState({ cover_image_preview: "", 'newGroupValues': { title: '', description: '', category: '', is_private: 'false', cover_image: '' } });
  ...
  onImageChange = event => {
    if (event.target.files && event.target.files[0]) {
      ...
      const params = { ...this.state.newGroupValues };
      params['cover_image'] = img;

      this.setState({ 'newGroupValues': params, cover_image_preview: URL.createObjectURL(img) })
  ...
  render() {
    ...
    return (
      <form className='compose-form simple_form' onSubmit={this.handleSubmit}>
        ...
        <div className='row'>
          <div>
            <img width={"50%"} src={this.state.cover_image_preview} />
            <h1>Cover Image</h1>
